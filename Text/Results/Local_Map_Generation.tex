...
\\ \\
The effect of the normalization is clearly visible, as the large scale range dependent brightness variation is reduced and the relative seabed structure becomes easier to interpret. At the same time, the background illumination is not perfectly uniform across all swaths. This is expected, since the EMA based method used here is only a lightweight approximation of the true illumination field rather than a full global estimate. Nevertheless, the result is sufficiently stable and visually consistent for real time swath processing and provides a practical compromise between quality and computational cost.
