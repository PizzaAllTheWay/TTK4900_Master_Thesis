...
This section presents the results obtained from the local map generation stage of the pipeline. The evaluation covers swath processing, probabilistic local map generation, and feature extraction, with emphasis on how raw side scan sonar measurements are progressively transformed into cleaner swaths, spatially consistent reflectivity maps, and finally compact landmark representations suitable for later SLAM processing.
\\ \\
The local map is an important intermediate representation, but it is not efficient to use the full dense map directly for repeated data association. As the mission grows, dense map comparison becomes increasingly expensive, and much of the image data contains background texture that is not useful for identifying repeatable landmarks. For this reason, the feature extraction stage compresses the relevant map information into a sparse set of landmarks with associated measurements, uncertainties, descriptors, and probabilistic characteristics. These landmarks form a compact global feature database, or sparse landmark map, that can be searched much faster than the full reflectivity map when proposing candidate matches or loop closures.
\\ \\
The results are assessed in two complementary ways. First, the intermediate and final outputs are examined qualitatively in order to evaluate whether the implemented methods produce physically meaningful swaths, candidate map support, local reflectivity maps, and landmark detections. Second, the computational performance is evaluated quantitatively through runtime, CPU usage, and memory consumption in order to determine whether the full processing chain remains lightweight enough for real-time deployment on embedded robotic platforms.
\\ \\
For the swath processing stage, the results focus on geometric correction, blind zone removal, intensity normalization, and the corresponding runtime behavior. For the map generation stage, the results focus on probabilistic pruning, persistent map state construction, normalized local map reconstruction, fast gap filling, and computational cost. For the feature extraction stage, the results focus on landmark segmentation, descriptor generation, height estimation, and the runtime impact of these operations.
\\ \\
It is important to note that the interpretation of the map generation and feature extraction results depends strongly on the quality of the underlying state estimate and on the availability of aiding measurements. When aiding measurements are available, the local maps are expected to remain spatially consistent and visually coherent, which also improves landmark stability. During periods of dead reckoning, accumulated estimator drift can gradually degrade map quality and reduce the reliability of extracted features. When aiding measurements return, estimator correction can also introduce visible distortions in the accumulated map. The following subsections therefore evaluate not only the algorithms themselves, but also how their practical output is influenced by the estimator conditions under which they operate.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\linewidth]{Pictures/Results/Local_Map_Generation/Feature_Extraction/Landmark_Measurements.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\linewidth]{Pictures/Results/Local_Map_Generation/Feature_Extraction/Estimated_Height.png}
...
\todo[inline]{EXTRA: on Feature extraction perfromance would also be nice to plot a subplot to number of landmarksdetected detected down bellow to inidcate where we detecetd landmarks jesjes. And also coment about that, ie it woudl be esier to comanet and tell what is going on to the vievwer jesjes that when landmarks + height it becomes more time consuming jesjes...}