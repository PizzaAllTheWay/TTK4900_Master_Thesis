...
\end{aligned}
$$
which defines the nominal, noise free kinematic model used in this work. It captures the translational and rotational motion of the AUV based solely on the IMUs accelerometer and gyroscope measurements, with attitude expressed through quaternion integration for smooth and singularity free orientation propagation.
\\ \\
...
\\ \\
Overall, the INS based motion model offers a compact and computationally efficient representation of the AUVs dynamics. While it omits detailed hydrodynamic effects, its balance of simplicity, generality, and compatibility with sensor fusion frameworks makes it a robust and practical choice for autonomous surface vehicle navigation and SLAM applications.



\subsubsection{Aiding Measurement Model}
Aiding measurements are external sensor measurements that are used to correct the drift from the inertial navigation model. The IMU can propagate the AUV state at high rate, but position, velocity, and attitude errors grow over time due to bias and noise. Sensors such as the depth sensor, GNSS, and DVL provide direct measurements of parts of the state and are therefore used to correct this drift when available.
\\ \\
Each aiding sensor is mounted at a known position and orientation relative to the body frame. The measurement model must therefore account for this fixed sensor offset before comparing the predicted state to the actual sensor measurement. In general, the predicted measurement is written as
$$
    \mathbf{z} = h(\mathbf{x}) + \mathbf{n}
$$
where $h(\mathbf{x})$ maps the estimated AUV state into the sensor measurement space, and $\mathbf{n}$ is zero mean Gaussian measurement noise with covariance $\mathbf{R}$.



\subsubsection{Depth Aiding Measurement Model}
The depth sensor measures the vertical position of the sensor head in the water column. Since the filter state describes the body origin, the known vertical offset between the body frame and the depth sensor must be included. The predicted depth measurement is therefore written as
\begin{equation}
    h_{\text{Depth}}(\mathbf{x}) =
    \begin{bmatrix}
        p_{b/O,z}^{n} + \left(R_b^n(\mathbf{q})\,\mathbf{p}_{\text{Depth}/b}^{b}\right)_z
    \end{bmatrix}
    \label{eq:aiding-measurement-model-depth}
\end{equation}
where $p_{b/O,z}^{n}$ is the estimated vertical position of the body origin, and $\mathbf{p}_{\text{Depth}/b}^{b}$ is the known stationary position of the depth sensor in the body frame.
\\ \\
The depth measurement model is
$$
    z_{\text{Depth}} = h_{\text{Depth}}(\mathbf{x}) + n_{\text{Depth}}
$$
with
\begin{equation}
    n_{\text{Depth}} \sim \mathcal{N}(0, R_{\text{Depth}}),
    \qquad
    \mathbf{R}_{\text{Depth}} =
    \begin{bmatrix}
        \sigma_z^2
    \end{bmatrix}
    \label{eq:aiding-measurement-model-depth-noise}
\end{equation}
where $\sigma_z^2$ is the depth sensor variance.



\subsubsection{GNSS Aiding Measurement Model}
The GNSS measurement provides absolute horizontal position when the AUV is at the surface. Since GNSS is not available when the AUV is submerged, it is treated as an additional aiding source rather than the main navigation reference. The GNSS antenna is mounted at a known offset from the body frame, so the predicted GNSS position is
\begin{equation}
    h_{\text{GNSS}}(\mathbf{x}) =
    \begin{bmatrix}
        \left(\mathbf{p}_{b/O}^{n} + R_b^n(\mathbf{q})\,\mathbf{p}_{\text{GNSS}/b}^{b}\right)_x \\
        \left(\mathbf{p}_{b/O}^{n} + R_b^n(\mathbf{q})\,\mathbf{p}_{\text{GNSS}/b}^{b}\right)_y
    \end{bmatrix}
    \label{eq:aiding-measurement-model-gnss}
\end{equation}
where $\mathbf{p}_{\text{GNSS}/b}^{b}$ is the known stationary GNSS antenna position in the body frame.
\\ \\
The GNSS measurement model is
$$
    \mathbf{z}_{\text{GNSS}} = h_{\text{GNSS}}(\mathbf{x}) + \mathbf{n}_{\text{GNSS}}
$$
with
\begin{equation}
    ...
    \begin{bmatrix}
        ...
        0 & \sigma_y^2
    \end{bmatrix}
    \label{eq:aiding-measurement-model-gnss-noise}
\end{equation}
where the covariance can be set from GNSS quality information, receiver output, or empirical measurements.



\subsubsection{DVL Aiding Measurement Model}
The DVL measures the AUV velocity relative to the seabed or water column. When bottom lock is available and seabed conditions are suitable, it provides a strong velocity aiding measurement. However, DVL data can degrade or drop out due to altitude, poor bottom lock, weak acoustic return, or unstable water conditions.
\\ \\
The DVL is usually not located exactly at the body origin, so the velocity at the DVL sensor is not always equal to the velocity at the AUV center of gravity. The rigid body velocity at the DVL position should ideally include the lever arm effect:
$$
    \mathbf{v}_{\text{DVL}/O}^{b}
    =
    R_n^b(\mathbf{q})\,\mathbf{v}_{b/O}^{n}
    +
    \boldsymbol{\omega}_{b/O}^{b} \times \mathbf{p}_{\text{DVL}/b}^{b}
$$
where $\mathbf{p}_{\text{DVL}/b}^{b}$ is the DVL position in the body frame, and $\boldsymbol{\omega}_{b/O}^{b}$ is the body angular velocity. This term is important when the DVL is mounted far from the body origin or when the AUV rotates quickly.
\\ \\
The ideal predicted DVL measurement would then be written as:
$$
    h_{\text{DVL,ideal}}(\mathbf{x}) =
    \begin{bmatrix}
        \left(R_b^{\text{DVL}}\left(R_n^b(\mathbf{q})\,\mathbf{v}_{b/O}^{n}
        + \boldsymbol{\omega}_{b/O}^{b} \times \mathbf{p}_{\text{DVL}/b}^{b}\right)\right)_x \\
        \left(R_b^{\text{DVL}}\left(R_n^b(\mathbf{q})\,\mathbf{v}_{b/O}^{n}
        + \boldsymbol{\omega}_{b/O}^{b} \times \mathbf{p}_{\text{DVL}/b}^{b}\right)\right)_y
    \end{bmatrix}
$$
However, the INS state model used in this thesis does not include the body angular velocity $\boldsymbol{\omega}_{b/O}^{b}$ as a state variable (See Equation \ref{eq:kinematics-motion-model-states} above). Therefore, the full lever arm compensation cannot be used directly without extending the state or estimating angular velocity separately. For this thesis, the DVL is assumed to be close enough to the body origin that the lever arm effect is small during normal operation. Short deviations during stronger rotations are treated as measurement disturbances and can be handled later by filtering and outlier rejection in the state estimation phase.
\\ \\
The simplified DVL measurement model used in this work is therefore:
\begin{equation}
    h_{\text{DVL}}(\mathbf{x}) =
    \begin{bmatrix}
        \left(R_b^{\text{DVL}} R_n^b(\mathbf{q})\,\mathbf{v}_{b/O}^{n}\right)_x \\
        \left(R_b^{\text{DVL}} R_n^b(\mathbf{q})\,\mathbf{v}_{b/O}^{n}\right)_y
    \end{bmatrix}
    \label{eq:aiding-measurement-model-dvl}
\end{equation}
where $R_b^{\text{DVL}}$ is the known rotation from body frame to DVL frame, and only the horizontal velocity components are used.
\\ \\
The DVL measurement model is
$$
    \mathbf{z}_{\text{DVL}} = h_{\text{DVL}}(\mathbf{x}) + \mathbf{n}_{\text{DVL}}
$$
with
\begin{equation}
    \mathbf{n}_{\text{DVL}} \sim \mathcal{N}(\mathbf{0}, \mathbf{R}_{\text{DVL}}),
    \qquad
    \mathbf{R}_{\text{DVL}} =
    \begin{bmatrix}
        \sigma_{v_x}^2 & 0 \\
        0 & \sigma_{v_y}^2
    \end{bmatrix}
    \label{eq:aiding-measurement-model-dvl-noise}
\end{equation}
where $\sigma_{v_x}^2$ and $\sigma_{v_y}^2$ describe the DVL velocity measurement uncertainty.



\subsubsection{Landmark Measurement Model}
The landmark measurement model establishes the mathematical relationship between the AUV pose and observable landmarks within the map. This formulation follows the classical range-bearing model commonly adopted in SLAM literature, as it provides a compact and geometrically consistent way of expressing 2D landmark observations \cite{sensor_fusion_book} \cite{side_scan_sonar_master_thesis}. The choice of the range-bearing model is motivated by its direct correspondence to the type of information available from the processed side scan sonar 2D map, where detected landmarks can be interpreted in terms of their relative distance and angle with respect to the vessels current position.  
\\ \\
The AUV state $\mathbf{x}$ is defined by the INS model presented earlier in Equation \eqref{eq:kinematics-motion-model-states}. From this full state, only the position component $\mathbf{p}_{b/O}^{n}$ and the attitude quaternion $\mathbf{q}$ are required for the landmark observation. The vehicle pose is expressed in the NED frame, while the landmark measurements are expressed relative to the local body frame of the sonar.  
\\ \\
Let the map contain a set of landmarks $\mathbf{m} = \{ m^1, m^2, \dots, m^N \}$, where each landmark $m^i = [m_x^i, m_y^i, 0]^\top$ is fixed in the NED frame. Although the AUV state $\mathbf{p}_{b/O}^{n} = [x, y, z]^\top$ is fully 3D, the side scan sonar and corresponding landmark map are inherently 2D, describing features on a locally planar surface. Consequently, only the horizontal components $(x, y)$ of both the vehicle position and the landmark locations are relevant for the range-bearing measurement, while variations in depth ($z$) are neglected.  
\\ \\
...
\end{equation}
This transformation converts the landmark position from the NED frame to the sensor local body frame, so that range and bearing are expressed relative to the vehicle pose and attitude. For an AUV, roll and pitch can affect the sonar geometry and should therefore not simply be ignored. In this model, the landmark observation is still represented as a 2D range bearing measurement, but the correction needed to form a geometrically stable 2D sonar map is handled earlier in the processing pipeline. This is discussed later in the \textit{``Local Map Generation''} chapter, under the \textit{``Swath Processing''} subsection, where pitch and roll correction are used to compensate the sonar swaths before feature extraction.
\\ \\
...
$$
    ...
    m_b^i = R_n^b(\mathbf{q}) \left( m^i - \mathbf{p}_{b/O}^{n} \right)
$$
where $R_n^b(\mathbf{q})$ transforms the landmark position from the navigation to the body frame. This angle changes with the AUV yaw and influences both reflection strength and measurement bias.  
\\ \\
