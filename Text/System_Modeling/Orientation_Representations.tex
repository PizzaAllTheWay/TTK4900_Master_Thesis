...
\end{figure}
\noindent
Despite this limitation, Euler angles remain useful in this work for logging, plotting, debugging, and result analysis, since roll, pitch, and yaw are easy to interpret directly. However, for an AUV the vehicle can experience larger and complex 3D motion, so Euler angles are not used as the main internal attitude representation. Instead, quaternions are preferred for state estimation and SLAM, since they avoid singularities and provide a more robust representation of orientation during submerged operation.
...
\\ \\
In practical applications, quaternions are widely used in estimators and sensors where singularity free orientation handling is critical. The IMU on the AUVs outputs attitude in quaternion form, which integrates naturally with sensor fusion algorithms such as the Error State Kalman Filter (ESKF) discussed later in the thesis. These algorithms rely on quaternion algebra to represent small attitude perturbations efficiently and avoid numerical instability associated with Euler angle singularities.  
\\ \\
...
\begin{equation}
    ...
    \end{bmatrix}
    \label{eq:system-modeling-quaternion-to-so3}
\end{equation}
\\ \\
\noindent
