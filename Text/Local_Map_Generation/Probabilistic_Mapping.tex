...
\todo[inline]{Somehow merge these two into one nified cahpter Slant Range Correction + Projection from Polar to Cartesian, maybe even remae it or something idk...}
...
\\ \\
Although the corrected slant ranges $r_{fbr,stb}$ and $r_{fbr,port}$ obtained from Equation \ref{eq:local-map-generation-first-bottom-return} account for vessel roll and pitch, they still describe distances along the acoustic path, not the horizontal ground distance. To produce a geometrically correct swath, each slant range sample is projected onto the seafloor plane assuming a locally flat seabed, which is a reasonable approximation for short range, high frequency side scan sonars.
\\ \\
...
\\ \\
Without this correction, near nadir regions would appear compressed and far range regions stretched, leading to geometric distortion in the mapped imagery. Using the previously estimated per channel transducer heights $h_{t,stb}$ and $h_{t,port}$ from Equation \ref{eq:local-map-generation-transducer-height-corrected}, the transformation is applied separately to the port and starboard channels:
\begin{equation}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\linewidth]{Pictures/Local_Map_Generation/Probabilistic_Mapping/slant_geometry.png}
