...
\\ \\
The feature extraction process operates directly on the normalized reflectivity map $M$, treating it as an intensity field analogous to a grayscale image. The objective is to detect, describe, and represent acoustically distinctive regions in a compact form that can later be matched across time. This involves applying image processing methods such as edge preserving filtering, adaptive thresholding, morphological filtering, and connected component analysis to isolate coherent acoustic responses from background texture and noise.
\\ \\
A central purpose of feature extraction is to reduce the amount of data that must be used during data association. The dense local map contains much more information than is needed for landmark matching, and matching directly against image-like map data becomes computationally expensive as the mission grows. Instead, the map is compressed into a sparse set of landmark observations, where each landmark stores only the information required for later matching and SLAM updates.
\\ \\
Each extracted feature is therefore represented by a compact set of characteristics, such as position, measurement uncertainty, local intensity structure, shape, shadow information, and descriptor weights. Over time, these landmark entries form a global feature database, or sparse global landmark map, which can be searched much more efficiently than the full reflectivity map. During data association, the system can first compare landmark descriptors and probabilistic characteristics to quickly reject unlikely matches and keep only the most probable loop closure candidates. This allows the SLAM front-end to perform global matching using compact feature information rather than repeatedly processing the full map representation.
\\ \\
Each detected landmark is defined by its spatial centroid $\mathbf{p}_L = [x_L, y_L]^\top$ in the local Cartesian frame, and characterized through a descriptor summarizing its local reflectivity, texture, and geometric properties. To interface with the SLAM framework, every landmark is expressed as a measurement in polar form:
$$
...
\noindent
The goal of this stage is therefore not only to detect features, but also to define a standardized and efficient data interface for the SLAM system. The expected input is the processed reflectivity map $M$, while the outputs are a set of landmark measurements $\mathbf{z}^i$ with associated uncertainty, descriptors, and probabilistic characteristics suitable for fast data association.
\\ \\
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1.0\linewidth]{Pictures/Local_Map_Generation/Feature_Extraction/landmark_measurement_example.png}
...
The detection of acoustic landmarks in the reflectivity map $M(x,y)$ follows the same general principles as classical image feature extraction. The map can be viewed as a grayscale image, where each pixel intensity represents the local acoustic reflectivity of the seafloor. Hence, identifying landmarks becomes a problem of detecting distinctive acoustic regions, such as bright echo responses and dark shadow regions, that stand out from the surrounding background.
\\ \\
The challenge arises from speckle noise, uneven illumination, and the fact that sonar landmarks are not always defined by simple intensity boundaries. A useful landmark may appear as a bright reflection followed by a nearby dark acoustic shadow, and this echo--shadow structure can vary in size, shape, and strength between observations. To address this, the reflectivity map is first preprocessed using an edge preserving filter to suppress high frequency noise while maintaining the important structural gradients that define echoes and shadows.
\\ \\
Subsequent segmentation is performed using a lightweight semantic region based approach rather than relying only on classical edge or threshold detection. The detector uses simple prior knowledge about side scan sonar imagery, mainly that object-like structures often appear as a local bright peak together with an associated dark shadow. This provides a more robust cue than treating bright and dark regions independently, while still remaining much faster and simpler than full ML based segmentation methods.
\\ \\
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\linewidth]{Pictures/Local_Map_Generation/Feature_Extraction/kernal_operation.png}
...
\paragraph{Semantic Region Segmentation} \mbox{}\\[0.5em] \noindent
After the reflectivity map has been filtered, the next step is to segment candidate landmark regions. Classical segmentation methods, such as global thresholding, adaptive thresholding, or Canny edge detection, are fast and simple, but they treat the image mostly as intensity values and boundaries. This is often not robust enough for side scan sonar data, because meaningful objects are usually expressed through a combination of bright echo responses and dark acoustic shadows rather than a single clean edge or thresholded region.
\\ \\
The segmentation used here is therefore based on a lightweight semantic interpretation of the sonar image. The main assumption is that a possible object-like landmark should contain a local bright response together with a nearby dark shadow response. The bright region is produced by a strong acoustic return from the object or seabed structure, while the nearby dark region is the shadow formed where the sonar beam is blocked or weakened. Together, the echo and shadow give a stronger cue than either part alone, making the segmentation less sensitive to uneven illumination and weak edges.
\\ \\
This approach can be understood as an informed region based segmentation method. It uses simple prior knowledge about side scan sonar image formation, but does not require a fully trained ML (Machine Learning) model or a large labelled dataset. In that sense, it lies between purely classical image processing and learning based segmentation. Classical methods are very fast but may fail under complex geometry or illumination, while ML methods such as CNN (Convolutional Neural Networks) based methods can be more accurate but are slower and require significant training data. The semantic region method is a practical middle ground, where a small amount of sonar specific knowledge is used to improve robustness while keeping the computation suitable for real-time processing.

\newpage

\noindent
It is also important that the semantic assumptions are not made too strict. If the detector relies on very specific object shapes, sizes, or intensity patterns, it may work well only for one dataset or one environment, but fail when the sonar angle, altitude, seabed type, or object geometry changes. For this reason, the semantic rule is kept deliberately simple and general. The echo-shadow relationship is used because it is a fundamental property of side scan sonar image formation, rather than a dataset specific pattern. This gives the method useful prior information while still allowing it to generalize across different sonar scenes.
\\ \\
The output of this stage is a binary candidate mask containing regions that are likely to correspond to physical acoustic structures. The purpose of semantic segmentation is therefore to provide a stronger and more meaningful initial candidate set for the rest of the feature extraction pipeline.
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\linewidth]{Pictures/Local_Map_Generation/Feature_Extraction/semantic_segmentation.jpg}
    \caption{Example of semantic segmentation. Some objects can be separated using simple colour or intensity differences, such as yellow peppers against a green background. However, objects with similar appearance to the background, such as green peppers among green leaves, are harder to isolate using only classical thresholding methods. Semantic segmentation uses additional prior information about what the object is and how it is expected to appear, allowing these regions to be separated more reliably. A similar idea is used for side scan sonar imagery, where the echo-shadow relationship provides the semantic cue for detecting candidate landmarks.\textsuperscript{\cite{semantic_segmentation}}}
    \label{fig:local-map-generation-semantic-segmentation}
\end{figure}

\newpage

\paragraph{Morphological filtering} \mbox{}\\[0.5em] \noindent
After Segmentation, the resulting binary mask of candidate pixels typically contains isolated noisy responses and small fragmented blobs caused by residual speckle or local contrast fluctuations. To refine the detection and enforce spatial consistency, the mask is processed using morphological operations that act directly on the pixel structure of the image.
\\ \\
...
\subsubsection{Landmark Height Estimation}
A landmark can be further described by estimating its physical height from the acoustic shadow it produces in the side scan sonar image. This height is not a direct sensor measurement, but an inferred geometric quantity derived from the relationship between the echo, the shadow, the vehicle altitude, and the landmark position. Such height information is useful later for landmark description, 2.5D map construction, and possible geometric correction of detected seabed structures.
\\ \\
The principle is based on acoustic shadowing. When the sonar beam hits an elevated seabed structure, part of the beam is reflected strongly, while the area behind the structure receives less acoustic energy. In the reflectivity map, this appears as a bright echo followed by a darker shadow. The taller the structure is, the longer this shadow tends to become for a given sonar geometry. Therefore, the shadow length can be used to infer an approximate landmark height.
\\ \\
Let $H$ denote the vehicle altitude above the seabed, $L$ the estimated shadow length, and $D$ the horizontal ground distance from the sonar to the landmark. Assuming locally flat seabed and a simplified 2D side view geometry, the sonar, the top of the object, and the end of the shadow form similar triangles. Under this model, the landmark height $\hat{h}_L$ is approximated as:
\begin{equation}
    \hat{h}_L = \frac{H L}{D + L}
    \label{eq:local-map-generation-landmark-height}
\end{equation}
where a longer shadow generally indicates a taller object, while a larger ground distance reduces the estimated height for the same shadow length.
\\ \\
The shadow length $L$ is estimated from the dark region associated with the landmark. Since sonar imagery contains speckle noise, uneven illumination, and unrelated dark regions, the shadow must be selected using local geometric and intensity consistency. Candidate shadow regions are identified as dark connected components near the landmark and filtered according to their relative size and spatial proximity. The selected shadow extent is then converted from image units to physical length using the local map resolution.
\\ \\
The resulting height estimate is assigned an uncertainty because it is derived from indirect image geometry rather than measured directly. A simple uncertainty model is:
\begin{equation}
    \sigma_{h_L}
    =
    \alpha_h
    ...
    \left(1 + \frac{D}{H}\right)
    \label{eq:local-map-generation-landmark-height-uncertainty}
\end{equation}
where $\alpha_h$ is a tunable confidence factor. This expression increases the uncertainty for large estimated heights, short shadows, and long-range or low-altitude viewing geometries, since these cases are more sensitive to segmentation errors and geometric assumptions.
\\ \\
The estimated height should therefore be interpreted as an approximate geometric reconstruction rather than a precise object measurement. The model assumes locally flat seabed, correct shadow association, stable pose and altitude estimates, and a simplified sonar projection geometry. Errors may arise from seabed slope, seabed roughness, grazing angle variation, acoustic distortion, map resolution, and incorrect shadow selection.
...
Each detected landmark must be represented in a compact and distinctive form so that it can be recognized again in later local maps. Directly comparing full reflectivity maps during data association is computationally expensive and inefficient, since most of the map contains background texture or information that is not useful for landmark matching. Instead, each landmark is compressed into a descriptor, which acts as a compact fingerprint of the feature.
\\ \\
A descriptor stores the most relevant characteristics of the landmark, such as its local intensity structure, texture, shape, position, and geometric properties. This allows data association to compare landmarks using small feature vectors rather than large image regions. Since landmark lookup and comparison may be performed many times during DA in SLAM, this compact representation is essential for fast candidate search, pruning, and loop closure detection.
\\ \\
Following the formulation in \textit{``Robust Imaging Sonar-based Place Recognition and Localization in Underwater Environments''} \cite{robust_descriptors}, the descriptor is divided into two complementary parts. The \textit{``strong descriptor''} captures local reflectivity and texture statistics, while the \textit{``weak descriptor''} captures coarse geometric and spatial information. This separation improves robustness by allowing appearance based information to drive the main matching process, while geometric information provides additional support for rejecting unlikely correspondences. 
...
The weak descriptor provides complementary geometric and low-level image cues that describe the landmark relative to the sonar frame:
\begin{equation}
    \mathbf{d}_L^{\text{weak}} =
    [\, A_L,\, \rho_L,\, \theta_L,\, \hat{h}_L,\, \sigma_{h_L},\, \bar{G}_L \,]
    \label{eq:local-map-generation-descriptors-weak}
\end{equation}
where $A_L$ is the landmark area, $(\rho_L,\theta_L)$ is the range-bearing measurement from Equation \ref{eq:local-map-generation-landmark-measurement}, $\hat{h}_L$ is the estimated landmark height from Equation \ref{eq:local-map-generation-landmark-height}, $\sigma_{h_L}$ is the corresponding height uncertainty from Equation \ref{eq:local-map-generation-landmark-height-uncertainty}, and $\bar{G}_L$ is the average internal gradient magnitude of the landmark region.
\\ \\
The landmark area is computed directly from the number of pixels in the connected region:
\begin{equation}
    A_L = N_L = |\mathcal{R}_L|
\end{equation}
providing a simple measure of the spatial extent of the detected feature. The polar coordinates $(\rho_L,\theta_L)$ describe the landmark position relative to the vehicle-centered sonar frame, while the estimated height adds a coarse geometric property of the detected structure. Height is included as a weak descriptor because it can be relatively consistent for the same physical object, but it is inferred indirectly from shadow geometry and is therefore less reliable than direct image measurements.
\\ \\
The final weak descriptor component, $\bar{G}_L$, measures the average strength of local intensity transitions inside the landmark region. First, Sobel derivatives are computed in the horizontal and vertical image directions:
\begin{equation}
    G_x = \frac{\partial M_s}{\partial x},
    \qquad
    G_y = \frac{\partial M_s}{\partial y}
\end{equation}
For each pixel inside the landmark region $\mathcal{R}_L$, the gradient magnitude is then computed as:
\begin{equation}
    G(x,y) = \sqrt{G_x(x,y)^2 + G_y(x,y)^2}
\end{equation}
The descriptor value is the average gradient magnitude over all landmark pixels:
\begin{equation}
    \bar{G}_L =
    \frac{1}{N_L}
    \sum_{(x,y)\in\mathcal{R}_L}
    G(x,y)
    \label{eq:local-map-generation-gradient-descriptor}
\end{equation}
This gives a compact measure of how strong the internal edges and intensity transitions are within the landmark. Smooth regions produce low gradient values, while structured regions with strong echo-shadow transitions produce higher values.
